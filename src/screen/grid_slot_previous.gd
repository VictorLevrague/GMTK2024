...
func _drop_data(at_position: Vector2, data: Variant) -> void:
    ...
    #Tooltip.body_popup(Rect2i(Vector2i.ZERO,Vector2i.ZERO), data)
...
func _on_mouse_entered():
    print("mouse entered in slot")
    #Signals.emit_signal("mouse_in_slot", self)
    if get_child_count() == 0:
        return
    print("name passed body: ", get_child(0).body_data.name)
    Tooltip.body_popup(Rect2i(Vector2i(global_position), Vector2i(size)), get_child(0))
