...
var inventory_bodies_load = ["res://src/body/resources/planets/earth.tres",
                            ...
                            "res://src/body/resources/planets/sister_planet1.tres",
...
func _ready() -> void:
    get_tree().root.get_child(1).get_node("Music4").play()
    for i in grids.size():
...
#Pas ouf, mais ça marchera pour aujourd'hui
    ...
    grid.get_child(3).add_child(body_grid1)
    ###
    var body_grid2 := GridBody.new()
    body_grid2.init(load("res://src/body/resources/fixed/comet_bottom_fixed.tres"))
    grid.get_child(10).add_child(body_grid2)
    ###
    var body_grid3 := GridBody.new()
    body_grid3.init(load("res://src/body/resources/fixed/comet_bottom_fixed2.tres"))
    grid.get_child(12).add_child(body_grid3)
    ###
    var body_grid4 := GridBody.new()
    body_grid4.init(load("res://src/body/resources/fixed/ufo_fixed.tres"))
    grid.get_child(13).add_child(body_grid4)
    ###
    var body_grid5 := GridBody.new()
    body_grid5.init(load("res://src/body/resources/fixed/black_hole_fixed.tres"))
    grid.get_child(16).add_child(body_grid5)
