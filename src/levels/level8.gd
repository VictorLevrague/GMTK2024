...
var inventory_bodies_load = ["res://src/body/resources/planets/earth.tres",
                            ...
                            "res://src/body/resources/star_dust.tres",
                            ...
                            "res://src/body/resources/comet_right.tres"]
...
func _ready() -> void:
    get_tree().root.get_child(1).get_node("Music3").play()
    for i in grids.size():
...
#Pas ouf, mais ça marchera pour aujourd'hui
    ...
    body_grid1.init(load("res://src/body/resources/fixed/ufo_fixed.tres"))
    grid.get_child(10).add_child(body_grid1)
    ###
    var body_grid2 := GridBody.new()
    body_grid2.init(load("res://src/body/resources/fixed/big_black_hole_fixed.tres"))
    grid.get_child(12).add_child(body_grid2)
    ###
    var body_grid3 := GridBody.new()
    body_grid3.init(load("res://src/body/resources/fixed/mini_sun_fixed.tres"))
    grid.get_child(23).add_child(body_grid3)
